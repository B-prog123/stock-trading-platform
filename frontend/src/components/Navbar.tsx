...
export default function Navbar() {
  ...
  return (
    <nav className="border-b border-[var(--border-color)] bg-[var(--bg-secondary)] shrink-0 z-40 relative">
      ...
      <div className="h-14 flex items-center justify-between px-4 lg:px-6 gap-4">
        ...
        <form onSubmit={handleSearch} className="relative flex-1 max-w-sm hidden md:block">
          ...
          <input
            type="text"
            placeholder="Search stocks eg: INFY, NIFTY..."
            className="w-full bg-[var(--bg-primary)] border border-[var(--border-color)] focus:border-blue-500 rounded-lg py-1.5 pl-9 pr-4 text-xs focus:outline-none transition-colors"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </form>

        {/* Right side: Indices + Icons */}
        <div className="flex items-center gap-2 shrink-0">
          {/* Market indices - only on wide screens */}
          <div className="hidden xl:flex items-center gap-4 text-xs font-semibold mr-2">
            {indexData.map(idx => (
              <div key={idx.name} className="flex gap-1.5 items-center">
              ...
              {isNotificationsOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 top-10 w-80 bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-xl z-50 rounded"
                  >
                    <div className="p-3 border-b border-[var(--border-color)] flex items-center justify-between">
                      <h4 className="font-semibold text-sm">Notifications</h4>
                      <button
                        onClick={clearNotifications}
                        className="text-[10px] uppercase text-[var(--text-secondary)] hover:text-red-500 flex gap-1 items-center"
                      >
          ...
          <div className="relative flex items-center">
            ...
            >
              ...
              <span className="text-sm font-medium hidden lg:block text-[var(--text-secondary)]">
              ...
              {isProfileOpen && (
                <>
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    className="absolute right-0 top-10 w-48 bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-xl z-50 rounded py-1"
                  >
                    <div className="px-4 py-2 border-b border-[var(--border-color)] mb-1">
                      <p className="text-xs text-[var(--text-secondary)]">{user?.email}</p>
                    </div>
                    <button onClick={() => { setActiveTab('profile-settings'); setIsProfileOpen(false); }} className="w-full text-left px-4 py-2 text-sm text-[var(--text-primary)] hover:bg-[var(--bg-primary)] transition-colors flex">
                      My Profile
                    </button>
      ...
      {/* Bottom row: All navigation tabs */}
      <div className="h-10 flex items-center px-4 lg:px-6 border-t border-[var(--border-color)]/50 overflow-x-auto scrollbar-hide gap-0">
        {navLinks.map(link => (
          <button
            key={link.id}
