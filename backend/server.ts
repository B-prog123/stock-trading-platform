...
const getBatchPrices = async (symbols: string[]): Promise<Record<string, { price: number; change: number }>> => {
  ...
  console.log(`[BACKEND] getBatchPrices for ${symbols.length} symbols: ${symbols.slice(0, 5).join(', ')}...`);
  ...
  try {
    ...
    if (Object.keys(result).length >= Math.floor(symbols.length * 0.7)) {
      console.log(`[BACKEND] ✅ NSE prices fetched: ${Object.keys(result).length}/${symbols.length}`);
      if (result['RELIANCE.NS']) console.log(`[BACKEND] RELIANCE.NS (NSE): ${result['RELIANCE.NS'].price}`);
      return result;
    }
  } catch (err) {
    console.warn("[BACKEND] NSE India fetch failed:", (err as Error).message);
  }
  ...
  if (remaining.length > 0) {
    try {
      console.log(`[BACKEND] 📡 Fetching ${remaining.length} symbols via Yahoo Finance Batch...`);
      ...
      if (result['RELIANCE.NS']) console.log(`[BACKEND] RELIANCE.NS (Yahoo): ${result['RELIANCE.NS'].price}`);
      if (result['RELIANCE']) console.log(`[BACKEND] RELIANCE (Yahoo): ${result['RELIANCE'].price}`);
    } catch (err: any) {
      console.warn(`[BACKEND] Yahoo Batch Quote failed:`, err.message);
    }
  }
...
app.get("/api/prices", async (req, res) => {
  try {
    ...
    const prices = await Promise.race([
      ...
      console.error("[BACKEND] /api/prices failed or timed out:", err.message);
      return {}; 
    });
    
    res.json(prices);
  } catch (err: any) {
    console.error("[BACKEND] Fatal error in /api/prices:", err.message);
    res.json({}); 
  }
});
